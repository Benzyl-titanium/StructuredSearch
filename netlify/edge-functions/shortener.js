...
export default async (request, context) => {
  ...
  if (url.pathname === '/api/shorten' && request.method === 'POST') {
    try {
      ...
      if (!longUrl) return new Response("Missing URL<br><a href='/'>Back to home</a>", { status: 400 });
  ...
  if (url.pathname.startsWith('/s/') && request.method === 'GET') {
    ...
    if (!id) return new Response("Invalid ID<br><a href='/'>Back to home</a>", { status: 404 });
    ...
    try {
      ...
      if (result.length > 0) {
        ...
        return new Response("Short URL not found<br><a href='/'>Back to home</a>", { status: 404 });
      }
    } catch (error) {
      return new Response("Database Error<br><a href='/'>Back to home</a>", { status: 500 });
    }
  }
